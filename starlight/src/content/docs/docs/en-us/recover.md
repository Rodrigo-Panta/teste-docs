...
> The opposite of [`Then`](then): it operates on the **failure rail**. `Recover` and its relatives bring the pipeline back to success — always in a **deliberate and explicit** way.
...
|---|---|
| just **observe** the error (log) without recovering | [`TapError`](tap) |
| **rewrite** the error, not recover | [`MapError`](map-errors) |
...
- [Errors and types](errors-and-types) — `IsTransient`, types and codes to condition the recovery
- [Remap errors · `MapError`](map-errors) — transform the error instead of recovering
- [Ensure · `Ensure`](ensure) — the opposite: take from success to failure
