...
|---|---|
| transform the value with something that **cannot fail** (build a DTO) | [`Map`](/teste-docs/docs/en-us/map) |
| **observe** the value (log, metric) without changing the rail | [`Tap`](/teste-docs/docs/en-us/tap) |
| keep **both** values, the old one and the new one | [`Zip`](/teste-docs/docs/en-us/zip) |
...
Every form exists for the synchronous `AxisResult<T>`, for `Task<AxisResult<T>>` and `ValueTask<AxisResult<T>>` pipelines. Each one also has a [`CancellationToken`-aware](/teste-docs/docs/en-us/cancellation) variant, where the delegate receives the token as its last parameter.
...
- [`Map`](/teste-docs/docs/en-us/map) — transform a value that cannot fail
- [`Ensure`](/teste-docs/docs/en-us/ensure) — guarantee an invariant inline (`RequireNotFound`, `WithValue`)
- [`Zip`](/teste-docs/docs/en-us/zip) — keep the old value *and* a new one
- [Errors and types](/teste-docs/docs/en-us/errors-and-types) — what an `AxisError` carries and the 12 categories
- [`Task` vs `ValueTask`](/teste-docs/docs/en-us/async-task-vs-valuetask) — which async form to chain
