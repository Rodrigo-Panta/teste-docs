...
|---|---|
| reduzir **N** resultados do **mesmo** tipo a um | [`Combine`/`All`](aggregate) |
| substituir o valor (não acumular) | [`Then`](then) |
...
- [Transformar · `Map`](map) — desestruturar a tupla com `(a, b) => …`
- [Agregar · `Combine`/`All`](aggregate) — para N resultados do mesmo tipo
- [`Task` vs `ValueTask`](async-task-vs-valuetask) — escolha do async no `ZipParallel`
